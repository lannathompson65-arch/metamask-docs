...
- **Seamless onboarding:** Create wallets or smart accounts as part of the invite or signup process, so users can jump into your game without additional steps.
- **Deferred wallet setup:** Delay login setups (such as OAuth) until players are ready for their first onchain transaction.
- **Prefunding capability:** Send tokens, NFTs, or in-game rewards to wallets before users claim them — perfect for welcome bonuses or preloaded assets.

## Use cases

- **Early wallet creation:** Assign wallets to players during onboarding or invitations without requiring immediate login.
- **Preloaded incentives:** Reward users with tokens, NFTs, or game items for offchain activities, redeemable when they access their wallets.
- **Enhanced gaming experiences:** Preload game items or promotional rewards to create a frictionless player journey.
...
This API only returns the Core Kit / SFA Key wallet addresses. To use the wallet addresses with the Embedded Wallet SDKs, you will need to use the `useCoreKitKey` flag when initializing the Unreal SDK.
