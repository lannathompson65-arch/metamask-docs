...
MetaMask Embedded Wallets supports multiple ways for users to add funds to their wallets via the Wallet Services UI. Users can:
...
The fiat on-ramp aggregator is chain-agnostic and supports over 100 payment methods across 50+ countries. To hide the top-up (funding) button from the Wallet Services widget, set `hideTopup: true` in `walletServicesConfig.whiteLabel`.

:::note

The fiat on-ramp supports both Externally Owned Accounts (EOA) and Account Abstraction (AA) accounts, as well as both EVM and Solana accounts.
